...
## 심화 선택: OpenMetadata 리니지 관점

OpenMetadata 참고 자료를 읽고 아래 중 하나를 3-5문장으로 정리해도 됩니다.

참고 자료:

```text
../../resources/openmetadata-lineage/README.md
```

선택 질문:

1. `commerce_category_daily` 지표가 이상하다면 upstream 중 어디부터 확인할까요?
2. Kafka topic `commerce-events`가 멈추면 어떤 downstream 자산이 영향을 받을까요?
3. OpenMetadata 같은 도구가 없으면 장애 영향 분석에서 무엇이 어려울까요?

...
즉 "검증을 실행해 서빙을 막는 일"은 `run-data-quality-checks.sh`가 담당하고, "어느 테이블이 실패했고 어떤 downstream BI에 영향을 주는지 설명하는 일"은 OpenMetadata 화면에서 다룹니다.

OpenMetadata는 선택 실습으로 분리해 두었습니다. Docker 리소스가 충분한 경우에만 아래처럼 별도 스택으로 실행합니다.
...
자세한 내용은 [openmetadata/README.md](./openmetadata/README.md)를 확인하세요. 시간이 부족하면 멘토 화면으로 table profile, test suite, lineage 관점만 짧게 확인합니다.
...
Serving 파트에서는 선택적으로 `./scripts/start-live-olist-events.sh`를 켜서 `ux-events`, `review-events`, `order-status-events`를 천천히 추가할 수 있습니다. 품질 게이트와 baseline count를 확인한 뒤에만 켜고, 기준값을 다시 맞춰야 할 때는 `./scripts/stop-live-olist-events.sh`로 중지합니다.

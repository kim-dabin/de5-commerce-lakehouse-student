...
## Windows에서 Streamlit BI 새로고침이 WinError 193으로 실패하는 경우
...
Windows Python 또는 PowerShell에서 `.sh` 스크립트를 Win32 실행 파일처럼 직접 실행한 상황입니다.
이 프로젝트의 운영 스크립트는 macOS/Linux/WSL2/Git Bash 기준 shell script입니다.

해결 방법입니다.

최신 코드를 받은 뒤 Streamlit BI를 다시 시작합니다.

```bash
git pull
bash ./scripts/start-streamlit-bi.sh
```

Streamlit 화면 안의 새로고침 버튼은 내부적으로 `bash`를 찾아 metric 스크립트를 실행합니다.
`bash`가 PATH에 없으면 WSL2 Ubuntu 또는 Git Bash terminal에서 실행하세요.

Git Bash를 쓰는 경우에는 Windows 경로 변환을 막기 위해 아래 환경 변수가 도움이 됩니다.

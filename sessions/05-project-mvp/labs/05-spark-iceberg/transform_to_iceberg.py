...
def main() -> None:
    ...
    ux_table = os.getenv(
        "PAIMON_UX_TABLE",
        f"{paimon_catalog}.{paimon_namespace}.ux_events_bronze",
    )
    review_table = os.getenv(
        "PAIMON_REVIEW_TABLE",
        f"{paimon_catalog}.{paimon_namespace}.review_current",
    )
    order_table = os.getenv(
        "PAIMON_ORDER_TABLE",
        f"{paimon_catalog}.{paimon_namespace}.order_current",
    )
    ...
    spark.table(ux_table).createOrReplaceTempView("ux_events_bronze")
    spark.table(review_table).createOrReplaceTempView("review_current")
    spark.table(order_table).createOrReplaceTempView("order_current")
    ...
    if os.getenv("SPARK_SHOW_PREVIEW", "1") == "1":
        spark.table(event_daily_table).orderBy("event_date", "event_type").show(50, truncate=False)
        spark.table(category_daily_table).orderBy("event_date", "category_code").show(
            50, truncate=False
        )
        spark.table(review_sentiment_table).orderBy("category_code", "sentiment").show(
            50, truncate=False
        )
    spark.stop()

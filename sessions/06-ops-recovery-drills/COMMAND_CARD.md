...
> 이 라운드는 실제 운영의 **설정값 오입력 사고**(retention 등 값을 `min.insync.replicas`에 잘못 넣어 쓰기가 막힌 사건)에서 **착안한 재현 데모**입니다. 실제 사고 그대로가 아니라, 그 원리(`ISR < min.insync.replicas`)를 로컬에서 보기 쉽게 broker를 내려 만든 것입니다.

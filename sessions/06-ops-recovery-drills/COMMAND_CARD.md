...
```text
실제 사례의 핵심은 min.insync.replicas 오설정으로 acks=all producer가 실패한 것입니다.
로컬 재현에서는 이 맥락을 보존하되, 화면에서 실패가 확실히 보이도록 max.message.bytes=64도 함께 낮춥니다.
따라서 Olist JSON producer는 메시지가 너무 커서 실패하고, 학생은 "입력 계층에서 막힌다"는 증거를 봅니다.
```
...
time travel은 바로 rollback을 실행하는 기능이 아니라, 먼저 "어느 snapshot까지 정상으로 볼 수 있는가"를 찾는 운영 기준점입니다.
수업에서는 rollback을 실행하지 않고 `RECOVERY_POINT_CANDIDATE` snapshot_id를 기록하는 것까지만 합니다.
```
